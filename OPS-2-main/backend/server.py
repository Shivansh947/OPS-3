...
DEMO_NEARBY_DRIVERS = [
    {"id": "nearby-rahul", "name": "Rahul Mehta", "vehicle_type": "Bike", "vehicle_number": "DL 3S CE 4821", "distance_km": 0.8, "eta_min": 3, "rating": 4.9},
    {"id": "nearby-neha", "name": "Neha Kapoor", "vehicle_type": "Bike", "vehicle_number": "DL 1Z AB 9137", "distance_km": 1.4, "eta_min": 5, "rating": 4.8},
    {"id": "nearby-imran", "name": "Imran Khan", "vehicle_type": "Auto", "vehicle_number": "DL 1R T 2846", "distance_km": 0.9, "eta_min": 4, "rating": 4.7},
    {"id": "nearby-simran", "name": "Simran Arora", "vehicle_type": "Car", "vehicle_number": "DL 8C AQ 6021", "distance_km": 1.2, "eta_min": 5, "rating": 4.9},
]
...
async def reverse_geocode(lat: float, lon: float):
    ...
    try:
        ...
        resp = requests.get(url, params={"lat": lat, "lon": lon}, timeout=8)
        if resp.status_code == 200:
            data = resp.json()
...
async def nearby_drivers(vehicle_type: str = "Bike"):
    drivers = [d.copy() for d in DEMO_NEARBY_DRIVERS if d["vehicle_type"] == vehicle_type]
    return {"drivers": drivers, "dispatch_count": len(drivers), "dispatch_strategy": "nearest-first"}
...
async def create_ride(data: RideCreate, current_user: dict = Depends(get_current_user)):
    ...
    nearby = sorted(
        [driver.copy() for driver in DEMO_NEARBY_DRIVERS if driver["vehicle_type"] == v_type],
        key=lambda driver: driver["distance_km"]
    )
    assigned = nearby[0] if nearby else None
    ...
    ride_doc = {
        ...
        "dispatch_count": len(nearby),
        "requested_driver_ids": [driver["id"] for driver in nearby],
        "status": "Accepted" if assigned else "Requested", # Requested, Accepted, Driver Arriving, Ride Started, Ride Completed, Cancelled
